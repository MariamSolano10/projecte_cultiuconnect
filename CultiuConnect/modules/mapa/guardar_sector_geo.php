<?php
/**
 * modules/mapa/guardar_sector_geo.php - Endpoint AJAX per actualitzar
 * la geometria d'un sector des del mapa.
 *
 * Metode: POST (JSON)
...
try {
    ...
    $check->execute([$id]);
    if (!$check->fetch()) {
        respond_json(404, ['success' => false, 'error' => 'Sector no trobat.']);
    }
    ...
    $backupFile = $backupDir . '/sector_' . $id . '.json';
    $backupData = [
        'id_sector' => $id,
    ...
    error_log('[CultiuConnect] guardar_sector_geo.php: ' . $e->getMessage());
